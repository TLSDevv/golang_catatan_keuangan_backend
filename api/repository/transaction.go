...
func (t *transactionRepo) List(ctx context.Context, tx *sql.Tx, limit int, page int) ([]domain.Transaction, error) {
	...
	if limit != 0 {
		if page != 0 {
			offset := (page - 1) * limit
			sql := `SELECT ` + structureTransaction + `FROM transactions WHERE deleted_at IS NOT NULL LIMIT $1 OFFSET $2`
			rows, err = tx.QueryContext(ctx, sql, limit, offset)
		} else {
			sql := `SELECT ` + structureTransaction + `FROM transactions WHERE deleted_at IS NOT NULL LIMIT $1`
			rows, err = tx.QueryContext(ctx, sql, limit)
		}
	} else {
		sql := `SELECT ` + structureTransaction + `FROM transactions WHERE deleted_at IS NOT NULL`
...
func (t *transactionRepo) GetByID(ctx context.Context, tx *sql.Tx, id int) (domain.Transaction, error) {
...
func (t *transactionRepo) Update(ctx context.Context, tx *sql.Tx, id int, transaction domain.Transaction) error {
